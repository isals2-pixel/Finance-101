...
One file per lesson. Markdown body = the NotebookLM source document (exported verbatim by `npm run export:pack -- NN`). YAML frontmatter = application data, never exported.
...
audio:
  file: 01-what-is-money.mp3   # under /public/audio; empty string until produced
  durationSec: 0               # filled after production
  generatedAt: ""              # date of NotebookLM generation
visual:
...
Body structure (SPEC_V2 §10): title header, opening problem (100-150 words), intuitive explanation (400-600), formal definitions (100-200), worked example (300-400, EUR, arithmetic written out), connections (150-250), common misconceptions (100-200). Target total 1,400-1,800 words until calibration says otherwise (CONTENT_PIPELINE.md).
...
`/public/audio/NN-slug.mp3`, mono, under 8 MB each. Referenced only by URL from lesson frontmatter. `scripts/build-feed.mjs` builds `podcast.xml` from lessons that have a non-empty audio file.

## Standing NotebookLM instruction

`/data/curriculum/audio-instruction.md`, under 1,500 characters, pasted into Audio Overview customization for every generation. Changing it is a content change: note it in CHANGELOG.md and regenerate affected audio per CONTENT_PIPELINE.md.
